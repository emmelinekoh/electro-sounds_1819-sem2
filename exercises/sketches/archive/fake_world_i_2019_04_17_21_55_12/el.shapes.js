/** ---------------------------------------------
 * Initialize Elements
 *  ---------------------------------------------
 */
function initBoxes() {
  boxes = [];
  for (let i = 0; i < 15; i++) {
    boxes.push(new Box(
      random(-100, 100),
      random(-100, 100),
      random(-100, 100)))

  }
}

function initBackground() {
  bgFrom = color(random(255), random(255), random(255), 80);
  bgTo = color(random(255), random(255), random(255), 80);
}

function initStarfield() {
  
  for (let i = 0; i < numStars; i++) {
    stars[i] = new Star(random(-width, width), random(-height, height), random(100, 1000));

  }
}

function initRiverB() {
  
  smooth();
  blank = createGraphics(5,5);
  blank.background(255,0,0);
  
}

/** ---------------------------------------------
 * Draw Elements
 *  ---------------------------------------------
 */

function drawStarfield() {
 //camera(sin(frameCount*0.01)*500,0,0,0,0,-400,0,1,0);
  stars.forEach((el) => {
    el.update();
    el.show();
  });
}

function drawMoon() {
  
  let spectrum = fft.analyze();
  console.log(spectrum[30]);
  
  push();
  ambientMaterial(255, 138, 225, 128);
  noStroke();
  translate(400, 0, -10);
  rotateX(angle * 0.01);
  rotateY(angle * 0.01);
  rotateZ(angle * 0.01);
  sphere(spectrum[30]/2);
  pop();
  angle += PI / 180 * 5

}

function drawLog() {
  let spectrum = fft.analyze();
  let level = amp.getLevel();
  vol = map(level,0,255,0,200)
  console.log(vol);
  
  push();
  stroke(0);
  strokeWeight(0.1)
  fill(200, 123, 20, 234);
  translate(-width / 2 + 225, height / 2 - 145);
  rotateX(PI/2*frameCount*0.05);
  // rotateY(PI/2*(frameCount*vol));
  cylinder(50, 500);
  pop();

}

function drawTrees() {
//-height / 2 + 85
  let spectrum = fft.analyze();
  let level = amp.getLevel();
  push();
  translate(-width / 2 + 170, -height / 2 + 85)
  for (let i = 0; i < 5; i++) {
    push();
    noStroke();
    translate(0, random(spectrum[0]), -i * 100);
    ambientMaterial(170, 255, 209);
    cone(40, 120);
    pop();
  }
  pop();

}

function drawGrass() {

  push();
  translate(-windowWidth / 2 - 300, -windowHeight / 2 + 10);
  for (let z = 0; z < 50; z++) {
    for (let x = 0; x < 40; x++) {
      push();
      translate(x * random(25), 0, -z * 10);
      normalMaterial();
      cone(7, 15);
      pop();
    }
  }
  pop();

  push();
  translate(windowWidth/2 + 200, -windowHeight/2 + 10);
  for (let z = 0; z < 200; z++) {
    for (let x = 0; x < 6; x++) {
      push();
      translate(-x * 5 * random(25), random(25), -z * random(10));
      normalMaterial();
      cone(7, 15);
      pop();
    }
  }
  pop();

}

function drawRiver() {
  push();
  strokeWeight(5);
  stroke(0);
  noFill();
  translate(width / 2, height / 2);
  rotateY(PI / 3);
  for (let y = 0; y < rows - 1; y++) {
    beginShape(TRIANGLE_STRIP);
    for (let x = 0; x < cols; x++) {

      vertex(x * scl, y * scl);
      vertex(x * scl, (y + 1) * scl);
    }
    endShape();
  }

  pop();
}

function drawRiverB() {
  
  let tubeRes = 40;
  let tubeX = [];
  let tubeY = [];
  let angle = TWO_PI / (tubeRes - 1);

  for (let i = 0; i < tubeRes; i++) {
    tubeX[i] = cos(angle * i);
    tubeY[i] = sin(angle * i);
  }
  
  translate(50,-100);
  noStroke();
  
  // rotateY(frameCount*0.001);
  // rotateX(frameCount*0.01);
    
  
  let xzScale = sin(frameCount*0.01)*100;
  let yScale = 10;
  
  
  texture(blank);
  for (let y = -10; y < 30; y++) {
    beginShape(TRIANGLE_STRIP);  
    for (let i = 0; i < tubeRes; i++) {
      let x = tubeX[i] * xzScale;
      let z = tubeY[i] * xzScale;
      
      ambientMaterial(20,123,142);
      vertex(x - sin(y + 1) * 20, (y+1) * yScale, z);
      fill(0,0,255);
      vertex((x) - sin(y) * 20, (y) * yScale, z);
      fill(255);
      vertex((x + 1) - sin(y + 1) * 20, (y + 1) * yScale, z);
      fill(0,255,0);
      vertex((x+1) - sin(y) * 20, (y) * yScale, z, 1,1);
    }
    endShape();  
  }
  
}


function drawBoxes() {
  // background(frameCount%255);
  noStroke();

  push();
  translate(-150, 0);
  rotateY(frameCount * 0.01)
  boxes.forEach(el => {
    el.draw();
  });
  pop();
  // camera(0, 0,-400 + sin(frameCount * 0.1) * 100, 0, 0, 0, 0, -1, -0.25);
}